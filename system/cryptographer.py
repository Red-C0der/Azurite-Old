...
def newkey(debug=0):
    ...
    try:
        ...
        PADDING = '{'
        if debug == 1:
            logger.write("d", "BLOCK_SIZE is ["+str(BLOCK_SIZE)+"]", lloc=lloc)
            logger.write("d", "PADDING is ["+str(PADDING)+"]", lloc=lloc)
        pad = lambda s: s + (BLOCK_SIZE - len(s) % BLOCK_SIZE) * PADDING
        if debug == 1:
            logger.write("d", "pad is ["+str(pad)+"]", lloc=lloc)
...
def encrypt(key, string, debug=0):
    ...
    try:
        ...
        if debug is 1:
            logger.write("d", "EncodeAES is ["+str(EncodeAES)+"]", lloc=lloc)
        cipher = AES.new(key)
        if debug is 1:
            logger.write("d", "cipher is ["+str(cipher)+"]", lloc=lloc)
        encoded = EncodeAES(cipher, string)
        if debug is 1:
            logger.write("d", "encoded is ["+str(encoded)+"]", lloc=lloc)
...
def decrypt(key, token, debug=0):
    ...
    try:
        ...
        pad = lambda s: s + (BLOCK_SIZE - len(s) % BLOCK_SIZE) * PADDING
        if debug is 1:
            logger.write("d", "pas is ["+str(pad)+"]", lloc=lloc)
        DecodeAES = lambda c, e: c.decrypt(base64.b64decode(e)).rstrip(PADDING)
        if debug is 1:
            logger.write("d", "DecodeAES is ["+str(DecodeAES)+"]", lloc=lloc)
        cipher = AES.new(key)
        if debug is 1:
            logger.write("d", "cipher is ["+str(cipher)+"]", lloc=lloc)
        decoded = DecodeAES(cipher, token)
        if debug is 1:
            logger.write("d", "decoded is ["+str(decoded)+"]", lloc=lloc)
